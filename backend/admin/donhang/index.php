        ...
        <div class="row d-flex" style="justify-content:right;">
            <div class="col-md-6">
                <form class="d-flex" method="post" action="index.php" role="search">
                    <input class="form-control me-2" type="search" name="search-order" placeholder="Tìm đơn hàng" aria-label="Search">
                    <button class="btn btn-outline-primary" name="search-order-btn" type="submit">Tìm</button>
                </form>
            </div>
        </div>
    </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        if(isset($_POST['search-order-btn'])){
                            if($_POST['search-order']!=null){
                               $search_order=$_POST['search-order'];
                               $query = "SELECT order_id, order_date, total_amount, `status` FROM orders WHERE order_id like '%$search_order%' OR order_date like '$search_order' OR status like '%$search_order%' ";
                            }else{
