                ...
                <div class="row">
                    <div class="col-md-12">
                                ...
                                if (isset($_SESSION['cart']) && !empty($_SESSION['cart'])) {
                                    ...
                                    if(isset($_POST['update-btn'])){
                                        foreach ($_SESSION['cart'] as $product_id => $quantity) {
                                            if (isset($_POST['quantity_'.$product_id.'']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
                                                $_SESSION['cart'][$product_id] = $_POST['quantity_'.$product_id.''];
                                            }
                                        }
                                    }
                                    foreach ($_SESSION['cart'] as $product_id => $quantity) {
                                        ...
                                        if ($result && mysqli_num_rows($result) > 0) {
                                            ...
                                            if (isset($_SESSION['cart'][$product_id]) && $_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['update-btn'])) {
                                            ...
                                            echo '<td><input type="number" value="'.$quantity.'" min="1" step="1" name="quantity_'.$product_id.'"  required></td>';
                        ...
                        if (isset($_SESSION['cart']) && !empty($_SESSION['cart'])) {
                            ...
                            echo ' <button type="submit" name="update-btn" class="btn btn-success" >Cập nhật giỏ hàng</button></form>';
                        }
                        ?>
                    </div>
                </div>
